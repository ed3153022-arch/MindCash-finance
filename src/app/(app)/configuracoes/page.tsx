...
export default function VereditoPage() {
  ...
  useEffect(() => {
    async function fetchVereditoData() {
      try {
        ...
        if (txError || limError) {
          console.warn("Tabelas não encontradas ou erro na busca. Usando dados padrão.");
        }
        ...
        setStats([
          { label: "Disciplina", value: (txs?.length || 0) > 0 ? 85 : 20 },
          { label: "Produtividade", value: (limits?.length || 0) > 0 ? 70 : 15 },
          { label: "Conhecimento", value: 90 },
          { label: "Resiliência", value: 65 },
          { label: "Autocontrole", value: Math.max(10, Math.round(100 - (totalGastoMes / totalLimiteMensal * 50))) },
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans pb-20 p-6">
       {/* ... O restante do JSX (Header, Teia, Tendências, Análise) continua igual ao anterior ... */}
       <div className="max-w-2xl mx-auto space-y-8 pt-20">
          <div className="space-y-2">
            ...
            <p className="text-zinc-500 text-[10px] font-black tracking-[0.3em] uppercase italic">Análise de Desempenho</p>
          </div>

          <div className="bg-yellow-400 p-6 rounded-[1.5rem] border-2 border-black flex items-center justify-between">
          ...
          <button onClick={() => router.push("/dashboard")} className="w-full py-6 text-zinc-700 font-black text-[9px] uppercase tracking-[0.5em] hover:text-white transition">
