...
export default function VereditoPage() {
  ...
  useEffect(() => {
    async function calculateMindCashCore() {
      try {
        setLoading(true);
        ...
        // --- TENDÊNCIA "FACA" ESTABILIZADA ---
        const numPontos = periodo === "dia" ? 24 : periodo === "semana" ? 7 : 30;
        let projection = [];
        const seed = Date.now();

        for (let i = 0; i < numPontos; i++) {
          // Ruído poligonal estável que nunca quebra o SVG
          const noise = (Math.sin(i * 1.5 + seed) * 20) + (Math.cos(i * 0.8) * 10);
          const tendenciaReal = (totalGanho - totalGasto) / totalLimite * 20;
          
          // Y centralizado com oscilação segura entre 10 e 90
          const yPos = Math.max(10, Math.min(90, 50 - tendenciaReal + noise));
    ...
    calculateMindCashCore();
  }, [periodo, getDataPoints]);

  const getTrendPath = () => {
    if (!trendData.length) return "";
    const step = 300 / (trendData.length - 1);
    // Uso de "L" para picos agudos (Estilo Faca)
    return trendData.reduce((acc, p, i) => {
      return i === 0 ? `M 0,${p.y}` : `${acc} L ${i * step},${p.y}`;
    }, "");
  };

  if (loading) return <div className="min-h-screen bg-black flex items-center justify-center text-yellow-400 font-black italic tracking-widest uppercase">[ Sincronizando Sistema ]</div>;
        ...
        <div className="bg-[#0A0A0A] p-10 rounded-[3rem] border border-white/5 shadow-2xl">
          ...
          <div className="h-44 w-full relative px-2">
            <svg viewBox="0 0 300 100" preserveAspectRatio="none" className="w-full h-full overflow-visible">
              <path d={getTrendPath()} fill="none" stroke="#facc15" strokeWidth="5" strokeLinecap="round" strokeLinejoin="round" style={{ filter: 'drop-shadow(0 0 15px rgba(250, 204, 21, 0.5))' }} />
              {trendData.length > 0 && (
                <circle cx="300" cy={trendData[trendData.length-1].y} r="5" fill="#facc15" className="animate-pulse" />
              )}
            </svg>
          </div>
          <p className="text-[7px] text-zinc-800 font-black mt-8 text-center tracking-[0.6em]">FUTURE PROJECTION ENGINE v4.3</p>
        </div>
