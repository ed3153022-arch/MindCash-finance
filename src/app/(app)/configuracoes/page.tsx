...
// Função de ruído para oscilações suaves e naturais
...
export default function VereditoPage() {
  ...
  useEffect(() => {
    async function calculatePreciseVeredito() {
      try {
        ...
        // Usamos Date.now() diretamente como semente
        const seed = Date.now() / 1000;
        const noiseArray = generateSmoothNoise(numPontos, seed, 15, periodo === "dia" ? 0.1 : 0.3);

        let projection = [];
        for (let i = 0; i < numPontos; i++) {
          // Centralizamos a linha em 50 e adicionamos a oscilação + tendência do saldo
          const tendenciaSaldo = (fluxoMensal / totalLimite) * 10;
          const yBase = 50 - tendenciaSaldo + noiseArray[i];
          projection.push({ x: i, y: Math.max(10, Math.min(90, yBase)) });
        }
  ...
  if (loading) return <div className="min-h-screen bg-black flex items-center justify-center text-yellow-400 font-black italic tracking-widest">RECALIBRANDO...</div>;
        ...
        <div className="bg-[#0A0A0A] p-10 rounded-[3rem] border border-white/5">
          <div className="flex justify-between items-center mb-12">
            ...
            <div className="flex bg-black p-1.5 rounded-2xl border border-white/10 shadow-inner">
              {["dia", "semana", "mês"].map(t => (
                <button key={t} onClick={() => setPeriodo(t as any)} className={`px-5 py-2 rounded-xl text-[9px] font-black transition-all ${periodo === t ? 'bg-yellow-400 text-black' : 'text-zinc-600'}`}>
                  {t}
                </button>
              ))}
            </div>
          ...
          <div className="h-44 w-full relative px-2">
            <svg viewBox="0 0 300 100" preserveAspectRatio="none" className="w-full h-full overflow-visible">
              <path d={getTrendPath()} fill="none" stroke="#facc15" strokeWidth="5" strokeLinecap="round" style={{ filter: 'drop-shadow(0 0 15px rgba(250, 204, 21, 0.4))' }} />
              {trendData.length > 0 && (
                <circle cx="300" cy={trendData[trendData.length-1].y} r="5" fill="#facc15" className="animate-pulse" />
              )}
            </svg>
          </div>
          <p className="text-[7px] text-zinc-800 font-black mt-8 text-center tracking-[0.6em]">FUTURE PROJECTION ENGINE v4.1</p>
        </div>
