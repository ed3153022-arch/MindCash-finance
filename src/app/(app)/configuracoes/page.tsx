  ...
  useEffect(() => {
    ...
    async function fetchVereditoGeral() {
      try {
        ...
        const entradasTotal = rawData.filter(t => t.type !== 'withdrawal').reduce((acc, t) => acc + Number(t.amount), 0);
        const saidasTotal = rawData.filter(t => t.type === 'withdrawal').reduce((acc, t) => acc + Math.abs(Number(t.amount)), 0);
        ...
        setResumo({ saldo: entradasTotal - saidasTotal, maiorGasto: categoriaTop, razao: saidasTotal / (entradasTotal || 1) });
        ...
        for (let i = 0; i <= totalPontos; i++) {
          ...
          tempPoints.push({ x: i * (300 / totalPontos), y: Math.max(8, Math.min(122, yPos)) });
        }
      ...
      <div className="max-w-xl mx-auto space-y-12 pt-8">
        ...
        <div className="bg-[#050505] p-10 rounded-[4rem] border border-white/5 relative overflow-hidden">
          <div className="flex justify-between items-center mb-16">
            <h4 className="text-[9px] font-black text-zinc-600 tracking-[0.3em] flex items-center gap-2">
              <TrendingUp size={12} className="text-yellow-500"/> TENDÊNCIA {periodo.toUpperCase()}
            </h4>
        ...
        <section className="bg-[#050505] p-10 rounded-[4rem] border border-white/5">
          ...
          <div className="grid grid-cols-2 gap-3 border-t border-white/5 pt-8">
            {Object.entries(radarStats).map(([key, val]) => (
              <div key={key} className="flex justify-between items-center bg-black/40 p-3 rounded-2xl border border-white/5">
                ...
                <span className="text-[10px] font-black text-yellow-400 italic">{val.toFixed(0)}%</span>
              </div>
            ))}
          </div>
        </section>
