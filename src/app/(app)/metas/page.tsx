    ...
    <>
      ...
      <div className="flex flex-col gap-2 w-full md:col-span-2">
        <div className="flex justify-between items-start">
          ...
          <button onClick={() => router.push("/dashboard")} className="text-[9px] font-black uppercase text-zinc-500 border border-white/10 px-4 py-2 rounded-full">← Voltar</button>
        </div>
        <button onClick={() => setShowModal(true)} className="bg-yellow-400 text-black py-5 rounded-2xl font-black text-[11px] uppercase tracking-widest mt-4 w-full md:max-w-xs">+ Definir Novo Limite</button>
      </div>

      {/* LISTA DE METAS - Ajustada para bordas e topo */}
      <div className="bg-[#111] pt-14 pb-10 px-8 rounded-[1.5rem] border border-white/5 w-full md:col-span-2">
        <h3 className="text-xl font-black italic uppercase text-white tracking-tighter mb-10">Meus Limites Ativos</h3>
            ...
            return (
              <div key={meta.id} className="bg-black/40 p-6 rounded-2xl border border-white/5 flex justify-between items-center group">
                ...
                <button 
                  ...
                  className="w-10 h-10 flex items-center justify-center rounded-full bg-red-500/20 text-red-500 border border-red-500/20 active:scale-95 transition"
                >
      ...
      {/* MODAL - Ajustado para não cortar o topo */}
      {showModal && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-md z-[100] flex items-center justify-center p-6">
          <div className="bg-[#111] w-full max-w-sm rounded-[1.5rem] pt-14 pb-10 px-8 border border-white/10 shadow-2xl text-white">
            <h2 className="text-2xl font-black italic uppercase text-white mb-8">Nova Meta</h2>
            
            <p className="text-[9px] font-black uppercase text-zinc-500 mb-4 italic">1. Categoria</p>
            <div className="grid grid-cols-3 gap-2 mb-8 max-h-[200px] overflow-y-auto pr-2">
              {MASTER_CATS.map(c => (
                <button key={c.nome} onClick={() => setCatSel(c.nome)} className={`p-3 rounded-xl border flex flex-col items-center ${catSel === c.nome ? "border-yellow-400 bg-yellow-400/10" : "border-white/5 bg-black/40"}`}>
            ...
            <div className="space-y-1 mb-10">
              <label className="text-[9px] font-black uppercase text-zinc-500 italic">2. Valor (R$)</label>
            ...
            <div className="flex flex-col gap-3">
              <button 
                onClick={async () => {
                  ...
                  // Primeiro removemos qualquer limite existente para evitar o erro de conflito
                  await supabase.from("goals").delete().eq("user_id", user?.id).eq("category", catSel);
                  
                  // Agora inserimos o novo
                  ...
                  if (error) {
                    alert("Erro ao salvar limite. Verifique sua conexão.");
                  } else {
                    setShowModal(false); setValor(""); loadMetas();
                  }
                }} 
                className="w-full bg-yellow-400 text-black py-5 rounded-2xl font-black uppercase text-[10px] tracking-widest"
              >
                Confirmar Limite
              </button>
