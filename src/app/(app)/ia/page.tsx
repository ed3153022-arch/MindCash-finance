...
// A Vercel e o seu PC vão ler a chave daqui (Sem expor o código da chave)
...
export default function AIAnalyticsPage() {
  ...
  async function processarIA() {
    ...
    try {
      ...
      // --- LÓGICA DE BLINDAGEM MATEMÁTICA ---
      ...
      // --- CHAMADA VIA OPENROUTER (GRATUITO) ---
      const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            { 
              role: "system", 
              content: `Você é o Cérebro, mentor financeiro de elite. 
              ### DADOS DO SISTEMA: ${JSON.stringify(dadosBlindados)}
              ### REGRAS:
              1. PRECISÃO: Use os valores de "resultado_final" e "porcentagem" da tabela acima.
              2. ANTI-ERRO: Se o valor começa com 2, nunca escreva 7.
              3. ALERTAS DE TETO: 
                 - < 80%: Elogio sofisticado.
                 - 80% a 100%: Alerta amarelo (teto próximo).
                 - > 100%: Puxão de orelha severo (limite ultrapassado) + 3 dicas de economia.
              Finalize com o JSON técnico: {"action": "insert", "amount": ${valorInput || 0}, "category": "NOME"}` 
            },
            { role: "user", content: promptDigitado }
          ],
          temperature: 0.1 // Mantém a IA focada e sem invenções
        })
      });
      ...
      setMessages(prev => [...prev, { role: "bot", text: "❌ Reiniciando conexão com o Cérebro..." }]);
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="p-6 border-b border-white/5 flex items-center gap-4 bg-black/50 backdrop-blur-md sticky top-0 z-50">
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest text-zinc-400">PRECISION v7.0 (FREE)</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-50">
            ...
            <p className="text-[10px] font-black uppercase italic tracking-widest text-yellow-400/70">Reset concluído. Pode falar, comandante.</p>
          </div>
        )}
